...
$maxRows_rsLoginRead = 1000;
$pageNum_rsLoginRead = 0;
if (isset($_GET['pageNum_rsLoginRead'])) {
  $pageNum_rsLoginRead = $_GET['pageNum_rsLoginRead'];
}
$startRow_rsLoginRead = $pageNum_rsLoginRead * $maxRows_rsLoginRead;

mysql_select_db($database_koneksi, $koneksi);
$query_rsLoginRead = "SELECT * FROM tb_login ORDER BY nik ASC";
$query_limit_rsLoginRead = sprintf("%s LIMIT %d, %d", $query_rsLoginRead, $startRow_rsLoginRead, $maxRows_rsLoginRead);
$rsLoginRead = mysql_query($query_limit_rsLoginRead, $koneksi) or die(mysql_error());
$row_rsLoginRead = mysql_fetch_assoc($rsLoginRead);

if (isset($_GET['totalRows_rsLoginRead'])) {
  $totalRows_rsLoginRead = $_GET['totalRows_rsLoginRead'];
} else {
  $all_rsLoginRead = mysql_query($query_rsLoginRead);
  $totalRows_rsLoginRead = mysql_num_rows($all_rsLoginRead);
}
$totalPages_rsLoginRead = ceil($totalRows_rsLoginRead/$maxRows_rsLoginRead)-1;
?>
<!DOCTYPE html>
<html>
<head>
...
<div class="w3-container w3-light-gray">
  ...
  <a href="<?php echo $logoutAction ?>" onClick="return confirm('Anda Yakin Ingin Keluar?')" style="text-decoration:none"><li><i class="fa fa-sign-out fa-fw"></i> Keluar</li></a>
   ...
   <div class="w3-border-bottom" style="padding-bottom:10px; margin-top:8px;"><strong><i class="fa fa-lock"></i> DATA ADMIN</strong><span class="w3-right"><a href="login_create.php" class="w3-tag w3-small" style="text-decoration:none"><i class="fa fa-plus fa-fw"></i> Tambah</a></strong></span></div>  
<?php if ($totalRows_rsLoginRead > 0) { // Show if recordset not empty ?>
  ...
  <table class="w3-table w3-striped w3-small w3-hoverable w3-bordered w3-border" id="hendra2" style="margin-top:8px;">
    ...
    <?php do { ?>
      <tr class="item2">
        ...
        <td><?php echo $row_rsLoginRead['nik']; ?></td>
        <td><?php echo $row_rsLoginRead['nama_lengkap']; ?></td>
        <td><?php echo $row_rsLoginRead['username']; ?></td>
        
        <td><?php echo $row_rsLoginRead['level']; ?></td>
        <td><a class="w3-tag w3-small w3-blue" style="text-decoration:none" href="login_update.php?id_login=<?php echo $row_rsLoginRead['id_login']; ?>">Ubah</a> <a class="w3-tag w3-small w3-red" style="text-decoration:none" onclick="return confirm('Anda Yakin Ingin Menghapus?')" href="login_delete.php?id_login=<?php echo $row_rsLoginRead['id_login']; ?>">Hapus</a></td>
      </tr>
      <?php } while ($row_rsLoginRead = mysql_fetch_assoc($rsLoginRead)); ?>
  </table><br>
  <?php } // Show if recordset not empty ?>
<?php
mysql_free_result($rsLoginRead);
?>
<?php if ($totalRows_rsLoginRead == 0) { // Show if recordset empty ?>
