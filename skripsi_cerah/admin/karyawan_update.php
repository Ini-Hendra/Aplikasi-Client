...
$colname_rsKaryawanUpdate = "-1";
if (isset($_GET['id_karyawan'])) {
  $colname_rsKaryawanUpdate = $_GET['id_karyawan'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_rsKaryawanUpdate = sprintf("SELECT * FROM tb_karyawan WHERE id_karyawan = %s", GetSQLValueString($colname_rsKaryawanUpdate, "text"));
$rsKaryawanUpdate = mysql_query($query_rsKaryawanUpdate, $koneksi) or die(mysql_error());
$row_rsKaryawanUpdate = mysql_fetch_assoc($rsKaryawanUpdate);
$totalRows_rsKaryawanUpdate = mysql_num_rows($rsKaryawanUpdate);
?>
<!DOCTYPE html>
<html>
<head>
...
<div class="w3-container w3-light-gray">
  ...
  <a href="<?php echo $logoutAction ?>" onClick="return confirm('Anda Yakin Ingin Keluar?')" style="text-decoration:none"><li><i class="fa fa-sign-out fa-fw"></i> Keluar</li></a>
   ...
   <div class="w3-border-bottom" style="padding-bottom:10px; margin-top:8px;"><strong><i class="fa fa-edit"></i> UBAH DATA KARYAWAN</strong></div>  
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
<div style="margin-top:8px;">
	<label>NIK</label>
    <input type="text" name="nik" value="<?php echo htmlentities($row_rsKaryawanUpdate['nik'], ENT_COMPAT, ''); ?>" size="32" class="w3-input w3-border w3-small" required />
    </div>
    
    <div style="margin-top:8px;">
	<label>Nama Karyawan</label>
    <input type="text" name="nama_karyawan" class="w3-input w3-border w3-small" required value="<?php echo htmlentities($row_rsKaryawanUpdate['nama_karyawan'], ENT_COMPAT, ''); ?>" size="32" />
    </div>
    
    <div style="margin-top:8px;">
	<label>Bagian</label>
    <input type="text" name="bagian" class="w3-input w3-border w3-small" required value="<?php echo htmlentities($row_rsKaryawanUpdate['bagian'], ENT_COMPAT, ''); ?>" size="32" />
    </div>
    
    <div style="margin-top:8px;">
	<label>Jenis Kelamin</label>
    <select name="jenis_kelamin" class="w3-input w3-border w3-small">
        <option value="Pria" <?php if (!(strcmp("Pria", htmlentities($row_rsKaryawanUpdate['jenis_kelamin'], ENT_COMPAT, '')))) {echo "SELECTED";} ?>>Pria</option>
        <option value="Wanita" <?php if (!(strcmp("Wanita", htmlentities($row_rsKaryawanUpdate['jenis_kelamin'], ENT_COMPAT, '')))) {echo "SELECTED";} ?>>Wanita</option>
      </select>
    </div>
  <table align="center" style="display:none">
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Id_karyawan:</td>
      <td><?php echo $row_rsKaryawanUpdate['id_karyawan']; ?></td>
    </tr>
    <tr valign="baseline">
    ...
    <tr valign="baseline">
      ...
      <td><select name="status">
        <option value="Aktif" <?php if (!(strcmp("Aktif", htmlentities($row_rsKaryawanUpdate['status'], ENT_COMPAT, '')))) {echo "SELECTED";} ?>>Aktif</option>
        <option value="Resign" <?php if (!(strcmp("Resign", htmlentities($row_rsKaryawanUpdate['status'], ENT_COMPAT, '')))) {echo "SELECTED";} ?>>Resign</option>
      </select></td>
    </tr>
    <tr valign="baseline">
  ...
  <input type="hidden" name="id_karyawan" value="<?php echo $row_rsKaryawanUpdate['id_karyawan']; ?>" />
</form>
             <br>
...
<?php
mysql_free_result($rsKaryawanUpdate);
?>
