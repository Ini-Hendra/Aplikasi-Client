...
$query_rsAdminRead = "SELECT * FROM tb_login ORDER BY `level` ASC";
$query_limit_rsAdminRead = sprintf("%s LIMIT %d, %d", $query_rsAdminRead, $startRow_rsAdminRead, $maxRows_rsAdminRead);
$rsAdminRead = mysql_query($query_limit_rsAdminRead, $koneksi) or die(mysql_error());
...
if (isset($_GET['totalRows_rsAdminRead'])) {
  ...
  $all_rsAdminRead = mysql_query($query_rsAdminRead);
  $totalRows_rsAdminRead = mysql_num_rows($all_rsAdminRead);
}
...
<div class="w3-container w3-light-gray">
  ...
  <a href="<?php echo $logoutAction ?>" onClick="return confirm('Anda Yakin Ingin Keluar?')" style="text-decoration:none"><li><i class="fa fa-sign-out fa-fw"></i> Keluar</li></a>
   ...
   <div class="w3-border-bottom" style="padding-bottom:10px; margin-top:8px;"><strong><i class="fa fa-lock"></i> DATA ADMIN</strong><span class="w3-right"><a href="admin_create.php" class="w3-tag" style="text-decoration:none"><i class="fa fa-plus fa-fw"></i> Tambah Data</a></span></div>  
  ...
  <table class="w3-table w3-striped w3-small w3-hoverable w3-bordered w3-border" id="hendra" style="margin-top:8px;">
    ...
    <?php do { ?>
      <tr class="item">
        ...
        <td><?php echo $row_rsAdminRead['level']; ?></td>
        <td><a class="w3-tag w3-small w3-green" style="text-decoration:none" href="admin_update.php?id_login=<?php echo $row_rsAdminRead['id_login']; ?>"><i class="fa fa-edit fa-fw"></i> Ubah Password</a> <a class="w3-tag w3-red w3-small" href="admin_delete.php?id_login=<?php echo $row_rsAdminRead['id_login']; ?>" onClick="return confirm('Anda Yakin Ingin Menghapus?\nUsername   : <?php echo $row_rsAdminRead['username']; ?>\nLevel           : <?php echo $row_rsAdminRead['level']; ?>')" style="text-decoration:none"><i class="fa fa-trash-o fa-fw"></i> Hapus</a></td>
      </tr>
