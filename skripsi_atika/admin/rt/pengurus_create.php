...
$ext = pathinfo($path, PATHINFO_EXTENSION);
  $insertSQL = sprintf("INSERT INTO tb_pengurus (id_pengurus, nama_lengkap, jabatan, kategori, nomor, no_hp, foto) VALUES (%s, %s, %s, %s, %s, %s, %s)",
...
copy($_FILES['fotonya']['tmp_name'],'foto_rt/'.$_POST['id_pengurus'].'.'.$ext);
  $insertGoTo = "pengurus_read.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_rsPengurusCreate = "-1";
if (isset($_GET['id_pengurus'])) {
  $colname_rsPengurusCreate = $_GET['id_pengurus'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_rsPengurusCreate = sprintf("SELECT * FROM tb_pengurus WHERE id_pengurus = %s", GetSQLValueString($colname_rsPengurusCreate, "text"));
$rsPengurusCreate = mysql_query($query_rsPengurusCreate, $koneksi) or die(mysql_error());
$row_rsPengurusCreate = mysql_fetch_assoc($rsPengurusCreate);
$totalRows_rsPengurusCreate = mysql_num_rows($rsPengurusCreate);
$IDPengurus = "123456789987654321";
?>
...
<div class="w3-large w3-border-bottom" style="padding-bottom:8px;">Tambah Data Pengurus</div>
<form action="<?php echo $editFormAction; ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
<div style="margin-top:8px;">
<input type="text" class="w3-input w3-border w3-small" required placeholder="Nama Lengkap" name="nama_lengkap" value="" size="32" />
</div>

<div style="margin-top:8px;">
<input type="text" class="w3-input w3-border w3-small" required placeholder="Jabatan" name="jabatan" value="" size="32" />
</div>
...
<div style="margin-top:8px;">
<input type="text" class="w3-input w3-border w3-small" required placeholder="Nomor HP" name="no_hp" value="" size="32" />
</div>

<div style="margin-top:8px;">
<div class="w3-small">Upload Foto Pengurus</div>
<input type="file" name="fotonya" id="fotonya" />
</div>
  <table align="center" style="display:none">
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Id_pengurus:</td>
      <td><input type="text" name="id_pengurus" value="<?php echo substr(str_shuffle($IDPengurus),0,7); ?>" size="32" /></td>
    </tr>
...
<br><?php
mysql_free_result($rsPengurusCreate);
?>
