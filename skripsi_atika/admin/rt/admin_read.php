...
$query_rsAdminRead = "SELECT * FROM tb_admin ORDER BY posting DESC";
$rsAdminRead = mysql_query($query_rsAdminRead, $koneksi) or die(mysql_error());
$row_rsAdminRead = mysql_fetch_assoc($rsAdminRead);
$totalRows_rsAdminRead = mysql_num_rows($rsAdminRead);
$nomor = 1;
?>
...
<div class="w3-container">
	<div class="w3-row">
		...
		<div class="w3-large w3-border-bottom" style="padding-bottom:8px;">Data Admin</div>
<?php if ($totalRows_rsAdminRead > 0) { // Show if recordset not empty ?>
  <div class="w3-row">
 	<div class="w3-col l3" style="padding-right:4px;"><a style="margin-top:8px; height:36px;" class="w3-btn w3-block w3-green w3-small" href="admin_create.php"><i class="fa fa-plus fa-fw"></i> Tambah Data</a></div>
	<div class="w3-col l3" style="padding-right:4px; padding-left:4px;"><input oninput="w3.filterHTML('#admin', '.admin', this.value)" placeholder="Pencarian Data" class="w3-input w3-border w3-center w3-small" style="outline:none;margin-top:8px;" autofocus></div>
    <div class="w3-col l6" style="padding-left:4px;"><div class="w3-border w3-border-yellow w3-pale-yellow w3-padding w3-small" style="margin-top:8px;">
        Total Data Admin<span class="w3-right"><?php echo $totalRows_rsAdminRead ?></span>
        </div></div>
</div>
  ...
  <table class="w3-table w3-striped w3-small w3-hoverable w3-bordered w3-border" id="admin" style="margin-top:8px;">
      ...
      <td class="w3-border-right">Username</td>
      <td class="w3-border-right">Password</td>
      
      <td class="w3-border-right w3-center">Level</td>
      <td class="w3-border-right w3-center">Ditambahkan</td>
      <td class="w3-center">Opsi</td>
    </tr>
    <?php do { ?>
      <tr class="admin">
        <td class="w3-border-right w3-center"><?php echo $nomor++; ?></td>
        <td class="w3-border-right"><?php echo $row_rsAdminRead['nama']; ?></td>
        <td class="w3-border-right"><?php echo $row_rsAdminRead['username']; ?></td>
        <td class="w3-border-right"><?php echo base64_decode($row_rsAdminRead['password']); ?></td>
        <td class="w3-border-right w3-center"><?php echo $row_rsAdminRead['level']; ?></td>
        <td class="w3-border-right w3-center"><?php echo $row_rsAdminRead['posting']; ?></td>
        <td class="w3-center"><a class="w3-tag w3-small w3-green" style="text-decoration:none" href="admin_update.php?id_admin=<?php echo $row_rsAdminRead['id_admin']; ?>">Ubah</a> <a class="w3-tag w3-small w3-red" style="text-decoration:none" onclick="return confirm('Anda Yakin Ingin Menghapus?')" href="admin_delete.php?id_admin=<?php echo $row_rsAdminRead['id_admin']; ?>">Hapus</a></td>
      </tr>
      <?php } while ($row_rsAdminRead = mysql_fetch_assoc($rsAdminRead)); ?>
  </table><br>

  <?php } // Show if recordset not empty ?>
<?php
mysql_free_result($rsAdminRead);
?>
