...
$query_rsJenisRead = "SELECT * FROM tb_jenis ORDER BY surat ASC";
$rsJenisRead = mysql_query($query_rsJenisRead, $koneksi) or die(mysql_error());
$row_rsJenisRead = mysql_fetch_assoc($rsJenisRead);
$totalRows_rsJenisRead = mysql_num_rows($rsJenisRead);
$nomor = 1;
?>
...
<div class="w3-container">
	<div class="w3-row">
        ...
        <div class="w3-large w3-border-bottom" style="padding-bottom:8px;">Data Jenis Surat</div>
<?php if ($totalRows_rsJenisRead > 0) { // Show if recordset not empty ?>
  <div class="w3-row">
 	<div class="w3-col l3" style="padding-right:4px;"><a style="margin-top:8px; height:36px;" class="w3-btn w3-block w3-green w3-small" href="jenis_create.php"><i class="fa fa-plus fa-fw"></i> Tambah Jenis Surat</a></div>
	<div class="w3-col l3" style="padding-right:4px; padding-left:4px;"><input oninput="w3.filterHTML('#jenis', '.jenis', this.value)" placeholder="Pencarian Data" class="w3-input w3-border w3-center w3-small" style="outline:none;margin-top:8px;" autofocus></div>
    <div class="w3-col l6" style="padding-left:4px;"><div class="w3-border w3-border-yellow w3-pale-yellow w3-padding w3-small" style="margin-top:8px;">
        Total Jenis Surat<span class="w3-right"><?php echo $totalRows_rsJenisRead ?></span>
        </div></div>
</div>

        
  <table class="w3-table w3-striped w3-small w3-hoverable w3-bordered w3-border" id="jenis" style="margin-top:8px;">
      <tr style="font-weight:bold" class="w3-hover-none">
      <td class="w3-border-right w3-center">No</td>
      <td class="w3-border-right">Nama Surat Penantar</td>
      <td class="w3-center">Opsi</td>
    </tr>
    <?php do { ?>
      <tr class="jenis">
        <td class="w3-border-right w3-center"><?php echo $nomor++; ?></td>
        <td class="w3-border-right"><?php echo $row_rsJenisRead['surat']; ?></td>
        <td class="w3-center"><a class="w3-tag w3-small w3-green" style="text-decoration:none" href="jenis_update.php?id_jenis=<?php echo $row_rsJenisRead['id_jenis']; ?>">Ubah</a> <a class="w3-tag w3-small w3-red" style="text-decoration:none" onclick="return confirm('Anda Yakin Ingin Hapus?\n<?php echo $row_rsJenisRead['surat']; ?>')" href="jenis_delete.php?id_jenis=<?php echo $row_rsJenisRead['id_jenis']; ?>">Hapus</a></td>
      </tr>
      <?php } while ($row_rsJenisRead = mysql_fetch_assoc($rsJenisRead)); ?>
  </table><br>
  <?php } // Show if recordset not empty ?>
<?php
mysql_free_result($rsJenisRead);
?>
<?php if ($totalRows_rsJenisRead == 0) { // Show if recordset empty ?>
  <div class="w3-row">
 	<div class="w3-col l3" style="padding-right:4px;"><a style="margin-top:8px; height:36px;" class="w3-btn w3-block w3-green w3-small" href="jenis_create.php"><i class="fa fa-plus fa-fw"></i> Tambah Jenis Surat</a></div>
	<div class="w3-col l3" style="padding-right:4px; padding-left:4px;"><input oninput="w3.filterHTML('#jenis', '.jenis', this.value)" placeholder="Pencarian Data" class="w3-input w3-border w3-center w3-small" style="outline:none;margin-top:8px;" autofocus></div>
    <div class="w3-col l6" style="padding-left:4px;"><div class="w3-border w3-border-yellow w3-pale-yellow w3-padding w3-small" style="margin-top:8px;">
