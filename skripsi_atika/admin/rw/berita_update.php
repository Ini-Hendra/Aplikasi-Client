...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  $updateSQL = sprintf("UPDATE tb_berita SET judul=%s, isi=%s, author=%s, posting=%s WHERE id_berita=%s",
  ...
  $Result1 = mysql_query($updateSQL, $koneksi) or die(mysql_error());
		...
		$updateSQL = sprintf("UPDATE tb_berita SET judul=%s, isi=%s, gambar=%s, author=%s, posting=%s WHERE id_berita=%s",
                       GetSQLValueString($_POST['judul'], "text"),
                       GetSQLValueString($_POST['isi'], "text"),
                       GetSQLValueString($_POST['id_berita'].'.jpg', "text"),
                       GetSQLValueString($_POST['author'], "text"),
                       GetSQLValueString($_POST['posting'], "date"),
                       GetSQLValueString($_POST['id_berita'], "text"));
  ...
  mysql_select_db($database_koneksi, $koneksi);
  $Result1 = mysql_query($updateSQL, $koneksi) or die(mysql_error());
  copy($_FILES['gambarnya']['tmp_name'],'../foto_berita/'.$_POST['id_berita'].'.jpg');
		}

  $updateGoTo = "berita_read.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_rsBeritaUpdate = "-1";
if (isset($_GET['id_berita'])) {
  $colname_rsBeritaUpdate = $_GET['id_berita'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_rsBeritaUpdate = sprintf("SELECT * FROM tb_berita WHERE id_berita = %s", GetSQLValueString($colname_rsBeritaUpdate, "text"));
$rsBeritaUpdate = mysql_query($query_rsBeritaUpdate, $koneksi) or die(mysql_error());
$row_rsBeritaUpdate = mysql_fetch_assoc($rsBeritaUpdate);
$totalRows_rsBeritaUpdate = mysql_num_rows($rsBeritaUpdate);
?>
...
<div class="w3-container">
	<div class="w3-row">
        ...
        <div class="w3-large w3-border-bottom" style="padding-bottom:8px;">Ubah Data Berita</div>
<form action="<?php echo $editFormAction; ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
<div style="margin-top:8px;">
<input type="text" class="w3-input w3-border w3-small" required placeholder="Judul Berita" name="judul" value="<?php echo htmlentities($row_rsBeritaUpdate['judul'], ENT_COMPAT, ''); ?>" size="32" />
</div>

<div style="margin-top:8px;">
<textarea name="isi" class="w3-input w3-border w3-small" required placeholder="Isi" style="max-height:200px;min-height:200px;max-width:100%;min-width:100%" cols="50" rows="5"><?php echo htmlentities($row_rsBeritaUpdate['isi'], ENT_COMPAT, ''); ?></textarea>
</div>

<div style="margin-top:8px;">
<div class="w3-small">Upload Foto Berita</div>
<img src="../foto_berita/<?php echo $row_rsBeritaUpdate['gambar']; ?>" width="147" height="97" /><br>
<input type="file" name="gambarnya" id="gambarnya" style="margin-top:6px;" />
</div>
  <table align="center" style="display:none">
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Id_berita:</td>
      <td><?php echo $row_rsBeritaUpdate['id_berita']; ?></td>
    </tr>
    
   
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Author:</td>
      <td><input type="text" name="author" value="<?php echo htmlentities($row_rsBeritaUpdate['author'], ENT_COMPAT, ''); ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Posting:</td>
      <td><input type="text" name="posting" value="<?php echo htmlentities($row_rsBeritaUpdate['posting'], ENT_COMPAT, ''); ?>" size="32" /></td>
    </tr>
  ...
  <input type="hidden" name="id_berita" value="<?php echo $row_rsBeritaUpdate['id_berita']; ?>" />
</form>
<br>
<?php
mysql_free_result($rsBeritaUpdate);
?>
